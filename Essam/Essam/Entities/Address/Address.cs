...
namespace Essam
{
    ...
    public class Address
    {
        ...
        public void Resolve(string country, string city, string street, string house, string zipCode)
        {
            ...
            if (dbCountry != null)
            {
                dbZipCode = Db.SQL<ZipCode>("SELECT z FROM ZipCode z WHERE Value = ? AND Country = ?", zipCode, dbCountry).First;
                if (dbZipCode == null)
                {
                ...
                if (dbCity != null)
                {
                    ...
                    if (dbStreet != null)
                    {
                        dbHouse = Db.SQL<House>("SELECT h FROM House h WHERE Value = ? AND Street = ?", house, dbStreet).First;
                        if (dbHouse == null)
                        {
